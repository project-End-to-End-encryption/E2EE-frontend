...
export async function send(conversationId, body, contentType = 'text', {attachments = []} = {}){
    ...
    const optimistic = {
        conversationId, seq: pendingSeq, clientMessageId, senderId: selfUserId,
        senderDeviceId: selfDeviceId, contentType, body: fullBody, attachments,
        sentAt, receivedAt: Date.now(), isRevoked: false, isOutgoing: true, state: 'sending'
    };
    ...
    try{
        ...
        await conversationRepo.setPreview(conversationId, {
            seq: result.seq, text: messageSync.previewTextFor(confirmed), senderId: selfUserId, sentAt
        });
        await conversationRepo.patch(conversationId, {
            lastSeq: result.seq, lastMessageAt: sentAt, lastReadSeq: result.seq, unreadCount: 0
        });
...
async function deliver({conversation, clientMessageId, fullBody, contentType, sentAt, attachments = []}){
    ...
    for (const device of devices) {
        try {
            const envelope = await SessionManager.encryptDirectMessage({
                toUserId: device.userId, toDeviceId: device.deviceId, plaintext
            });
    ...
    const ack = await rpc(SOCKET_EVENTS.MESSAGE_SEND, {
        conversationId, clientMessageId, contentType, archive, envelopes, attachments, sentAt
    });
...
export async function flushOutbox(){
    ...
    for (const entry of pending) {
        ...
        if (row && row.attempts > 10) { await outboxRepo.remove(entry.clientMessageId); continue; }
        try {
            ...
            await deliver({ conversation, clientMessageId: entry.clientMessageId, fullBody: entry.body, contentType: entry.contentType, attachments: entry.attachments ?? [], sentAt: entry.body.sentAt });
            await outboxRepo.remove(entry.clientMessageId);
            sent += 1;
        } catch (error) { console.debug('[composer] outbox retry failed:', entry.clientMessageId, error.code); }
    }
    return sent;
}

export const sendText = (conversationId, text) => send(conversationId, { kind: 'text', text }, 'text');
export const sendMedia = (conversationId, { kind, storageKey, mediaKeyBase64, ivBase64, mimeType, byteSize, durationMs, filename, width, height }) => send(conversationId, { kind, storageKey, mediaKeyBase64, ivBase64, mimeType, byteSize, durationMs, filename, width, height }, kind === 'voice' ? 'audio' : kind);
export const sendAttachments = (conversationId, { text = '', bodyAttachments = [], serverAttachments = [] }) => send(conversationId, { kind: 'media', text, attachments: bodyAttachments }, 'media', { attachments: serverAttachments });
export const sendCallEvent = (conversationId, { callId, action, media, durationMs }) => send(conversationId, { kind: 'callEvent', callId, action, media, durationMs }, 'call');
