...
export const mbkStore = {
    ...
    async set(rawBytes, { persist = true } = {}) {
        ...
        if (persist) {
            try { await keyStorage.saveMasterBackupKey(key); }
            catch (error) { console.warn('[mbkStore] could not persist MBK:', error.message); }
        }
        mbkKey = key;
        resolveWaiters(key);
        return key;
    ...
    async setKey(cryptoKey, { persist = true } = {}) {
        ...
        if (persist) {
            try { await keyStorage.saveMasterBackupKey(cryptoKey); }
            catch (error) { console.warn('[mbkStore] could not persist MBK:', error.message); }
        }
        mbkKey = cryptoKey;
    ...
    async clear({ persistent = false } = {}) {
        if (persistent) {
            try { await keyStorage.deleteMasterBackupKey(); }
            catch (error) { console.warn('[mbkStore] could not delete persisted MBK:', error.message); }
        }
        mbkKey = null;
        const pending = waiters;
        waiters = [];
